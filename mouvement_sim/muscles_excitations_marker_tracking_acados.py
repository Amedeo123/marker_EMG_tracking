...
def prepare_ocp(
    ...
    objective_functions.add(Objective.Lagrange.TRACK_MUSCLES_CONTROL, weight=0.001, target=excitations_ref)

    if use_residual_torque:
        objective_functions.add(Objective.Lagrange.MINIMIZE_TORQUE, weight=10)

    if kin_data_to_track == "markers":
        objective_functions.add(Objective.Lagrange.TRACK_MARKERS, weight=1, target=marker_ref)
...
if __name__ == "__main__":
    ...
    sol = ocp.solve(solver=Solver.ACADOS,
                    solver_options={"qp_solver": "PARTIAL_CONDENSING_HPIPM", "integrator_type": "IRK",
                                    "nlp_solver_max_iter": 400, "sim_method_num_steps": 1})
    # sol = ocp.solve(solver=Solver.ACADOS, solver_options={"qp_solver":"FULL_CONDENSING_QPOASES"})
