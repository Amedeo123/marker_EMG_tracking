...
def prepare_ocp(
    ...
    excitation_min, excitation_max, excitation_init = -1, 1, 0.1
    torque_min, torque_max, torque_init = -100, 100, 0

    # Add objective functions
    objective_functions = ObjectiveList()

    objective_functions.add(Objective.Lagrange.TRACK_MUSCLES_CONTROL, weight=0.01, target=excitations_ref)
    ...
    #x_bounds[0].min[:, 0] = state_ekf[:2 * biorbd_model.nbQ(), 0] 
    #x_bounds[0].max[:, 0] = state_ekf[:2 * biorbd_model.nbQ(), 0] 

    # Initial guess
    x_init = InitialConditionsList()
    # x_init.add([0] * biorbd_model.nbQ() + [0] * biorbd_model.nbQdot())
    x_init.add(state_ekf[:2 * biorbd_model.nbQ(),:], interpolation=InterpolationType.EACH_FRAME)
...
if __name__ == "__main__":
    ...
    # marker_ref[0, 14, 25] = np.nan
    # marker_ref[0, 20, 20] = np.nan
    # marker_ref[0, 2, 3] = np.nan
    ...
    # sol = ocp.solve(solver=Solver.ACADOS, solver_options={"qp_solver":"PARTIAL_CONDENSING_HPIPM"})
    ...
    ocp.save(sol, "activation_driven_ipopt.bo")
