...
def prepare_ocp(
    ...
    objective_functions.add(Objective.Lagrange.MINIMIZE_STATE, states_idx=[1], weight=10)
   ...
   # x_bounds[0].min[:, 1:] = -100
   # x_bounds[0].max[:, 1:] = 100

    x_bounds[0].min[:, 0] = state_ekf[ :, 0]
    x_bounds[0].max[:, 0] = state_ekf[ :, 0]
    ...
    init_residual_torque = np.concatenate((np.ones((biorbd_model.nbGeneralizedTorque(), n_shooting_points))*0,
                                           excitations_ref))
    if use_residual_torque:
        u_bounds.add(
            [
...
if __name__ == "__main__":
    ...
    x_init = mat_contents["x_init"][: 2 * biorbd_model.nbQ(), :]
    ...
    #                                                          })
    tic = time()
    sol = ocp.solve(solver=Solver.ACADOS, solver_options={"qp_solver":"PARTIAL_CONDENSING_HPIPM","integrator_type":"IRK", "nlp_solver_max_iter":400, "sim_method_num_steps":1})
    #sol = ocp.solve(solver=Solver.ACADOS, solver_options={"qp_solver":"FULL_CONDENSING_QPOASES","sim_method_num_steps":3})
    #sol = ocp.solve(solver=Solver.ACADOS, solver_options={"qp_solver":"FULL_CONDENSING_HPIPM", "sim_method_num_steps":3})
    toc_ac = time() - tic
   ...
   # plt.show()
    ...
    ocp.save(sol, "marker_tracking_residual_torque_acados.bo")
    ocp.save_get_data(sol, "sol_marker_activation_tracking_acados.bob")
