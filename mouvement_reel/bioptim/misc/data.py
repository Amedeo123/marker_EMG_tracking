    ...
    def get_data_object(ocp, V, phase_idx=None, integrate=False, interpolate_nb_frames=-1, concatenate=True):
        ...
        for i, nlp in enumerate(ocp.nlp):
            if nlp.control_type == ControlType.CONSTANT:
                # if ocp.solver_type.name == 'ACADOS':
                #     offsets.append((offsets[i] * (nlp.ns + 1)) + nlp.nx * (nlp.ns + 1) + nlp.nu * (nlp.ns))
                # else:
                offsets.append(offsets[i] + nlp.nx * (nlp.ns + 1) + nlp.nu * (nlp.ns))
            elif nlp.control_type == ControlType.LINEAR_CONTINUOUS:
                # if ocp.solver_type.name == 'ACADOS':
                #     offsets.append((offsets[i] * (nlp.ns + 1)) + (nlp.nx + nlp.nu) * (nlp.ns + 1))
                # else:
        ...
        for i in phase_idx:
            ...
            #         if nlp.control_type == ControlType.CONSTANT:
            #             V_phase = np.concatenate((V_phase,
            #                                       np.array(V_array[offsets[i] + ((nlp.nx + nlp.nu + offsets[i]) * k):
            #                                                        (nlp.nx + nlp.nu + offsets[i]) * (k+1)])))
            #     V_phase = np.concatenate((V_phase,
            #                               np.array(V_array[offsets[i+1] - ((nlp.nx + nlp.nu + offsets[i])):
            #                                                offsets[i+1]])))
            #
            #         # elif nlp.control_type == ControlType.LINEAR_CONTINUOUS:
            #         #     V_phase = np.concatenate(
            #         #         (V_phase, np.array(V_array[offsets[i] + ((nlp.nx + nlp.nu + offsets[i]) * k)
            #         #                                    :(nlp.nx + nlp.nu + offsets[i]) * (k + 1)])))
            # else:
