...
tol_final = 6
tol_final += 1
x_ip = np.ndarray((tol_final - tol_init + 1, biorbd_model.nbQ()*2, n_shooting_points+1))
...
for i in range(tol_final - tol_init):
    for s in range(biorbd_model.nbQ() * 2):
        ...
        for k in range(n_shooting_points):
            sum = sum + ((x_ip[i, s, k] - sol_simulate_single_shooting_ip[i]['x'][k*28 + s])**2)
        RMSE_ip[i, s] = np.sqrt(sum/n_shooting_points)

sio.savemat("RMSE_ip_nQ.mat",
            {"RMSE_ip": RMSE_ip})

RMSE_ip = np.ndarray((tol_final - tol_init, n_shooting_points))
sum = 0
for i in range(tol_final - tol_init):
    for k in range(n_shooting_points):
        sum = 0
        for s in range(biorbd_model.nbQ() * 2):
            sum = sum + ((x_ip[i, s, k] - sol_simulate_single_shooting_ip[i]['x'][k*28 + s])**2)
        RMSE_ip[i, k] = np.sqrt(sum/biorbd_model.nbQ() * 2)

sio.savemat("RMSE_ip_ns.mat",
            {"RMSE_ip": RMSE_ip})